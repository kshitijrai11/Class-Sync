...
let totalTeachers = ${totalTeachers};

function calculatePercentage(subjectStudents, totalStudents) {
    return Math.absolute((subjectStudents / ${totalStudents}) * 100);
}

function calculatePercentage1(classStudents, totalStudents) {
    return Math.absolute((classStudents / totalStudents) * 100);
}
        ...
        <div class="main-screen_t xl-col-8">
            <div class="content_t">
                <div class="button-container_t">
                    ...
                    <button id="totalCandidatesBtn" class="download-btn_t" onclick="document.getElementById('csvFileInput').click();">Upload CSV</button>
                </div>

             <div class="button-container_t">
                 <button id="deleteAllBtn" class="download-btn_t" onclick="deleteAllStudents();">Delete All</button>
             </div>
            <div class="button-container_t">

                   <input type="file" id="csvCenterInput" accept=".csv" style="display: none;" onchange="handleCenterUpload(event)">
                   <button class="download-btn_t" onclick="document.getElementById('csvCenterInput').click();">Upload Center List</button>
               </div>
                ...
                <div class="cards-container" id="cardsContainer2">
                    ...
                    <div class="card" onclick="location.href='/classsevenstudents'">
                        ...
                        <div class="card-content">
                            ...
                            <h2 class="count" data-target="5">5</h2>
    ...
    <script>
       function handleFileUpload(event) {
           ...
           if (file && file.type === "text/csv") {
               ...
               formData.append('csvFile', file); // Ensure 'csvFile' matches your @RequestParam in Spring
       ...
       function deleteAllStudents() {
           if (confirm('Are you sure you want to delete all student records? This action cannot be undone.')) {
               ...
               .catch(error => {
                   console.error('Error:', error);
                   alert('There was an error deleting student records: ' + error.message);
               });
           }
       }

function handleCenterUpload(event) {
    const file = event.target.files[0];
    if (file && file.type === "text/csv") {
        const formData = new FormData();
        formData.append('csvFile', file);

        fetch('/uploadCenterList', {
            method: 'POST',
            body: formData
        })
        .then(response => {
        ...
        .then(data => alert('Center list uploaded successfully!'))
